...
## CLI sync vs push — quality of the code extract (2026-09)
- Two ingestion paths with very different extract quality: `push` takes `.md` files (e.g. generated by pr-aquila-ap-v2's `blueprint-extract.ts`: filtered imports + full declaration body via bracket balancing) while `sync` reads the skill TSV index and `extractExcerpt` (`sync.core.ts`) only captures the annotation block + the declaration line → 1 line of code per blueprint (39/42 LD blueprints vs median 26 lines for Aquila's 85).
- Fix direction: make `extractExcerpt` extend to the full balanced body (reuse `countBrackets` string-literal-aware logic from `pr-aquila-ap-v2/.claude/skills/blueprint/blueprint-extract.ts`), cap length to avoid dumping whole files, then re-run `sync` to update existing slugs. `sync.core.ts` is a `.core.ts` → 100% test coverage required.
- Inspecting prod data: local docker DB is empty; use `~/.theodo-blueprints/config.json` (server+token) against the Vercel API. `GET /api/blueprints?limit` is capped at 100 (128 total → paginate); filter by `projectId` (UUIDs from `GET /api/projects`) to split per-project.

## Full-body blueprint extracts (2026-09)
- `extractExcerpt` (cli `sync.core.ts`) now follows the full declaration body: bracket-balanced (braces/parens/brackets), string/comment aware (`scanLine`), Python indentation blocks supported, capped at `MAX_EXCERPT_LINES` (150). Re-run `sync` to push corrected content to existing slugs.
- Upstream skill `future-of-software/skills/blueprint` gained `extract.sh` (pure shell + awk): one frontmatter `.md` per `@Blueprint` with the full body, ready for `theodo-blueprints push --dir`. Gotchas: apostrophes inside the single-quoted awk program break shell quoting (pass `'` via `-v sq="'"`); `read` with `IFS=$'\t'` COLLAPSES empty fields (tab is IFS-whitespace) — use a non-whitespace separator (`\034`) for TSV rows; always give awk an explicit `</dev/null` inside read-loops or it eats the loop's stdin on empty file args; `gensub` is gawk-only (BSD awk: `match` + `RSTART`/`RLENGTH`).
- LD TSV index lives at `docs/standards/blueprints.tsv` (44 rows, `atomic-permission` not yet in the registry — next sync creates it).
- 2026-09-16: prod (blueprints-api.vercel.app) started failing ALL DB reads mid-session ("Failed query" on trivial selects with pre-existing columns) — Neon incident or deployment drift from the WIP; registry updates are blocked until it recovers.
