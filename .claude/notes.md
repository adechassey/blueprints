...
- A blueprint seeded in March that no code annotates any more (`service-admin-lifecycle-transition`) was deleted from prod; its markdown still lives at `webapp/src/assets/blueprints/service-admin-lifecycle-transition.md` if it ever needs to come back.
