...
- Validate extractor changes by sweeping EVERY annotation site (`@Blueprint` + `@FollowsBlueprint`) of real repos, old vs new, not just the index rows: 15/1546 excerpts changed in one synced repo and 238/1078 in the other, all reviewed. Filter the changed ones on "last line is not `;`/`}`/`)`" to spot cut excerpts, and on "a later line at the declaration indent" to spot over-grabs.
...
- Re-sync is cheap: `updateBlueprint` only creates a version when content changed. Dry-run from the source repo root (TSV locations are cwd-relative): `<worktree>/cli/node_modules/.bin/tsx <worktree>/cli/src/index.ts sync <tsv> --project <project-slug> --repo <owner/repo> --dry-run`. Neither synced repo commits `docs/blueprints.tsv`: generate it with the skill's `index.sh` into a scratch dir.
...
- `sync` now classifies each row from its exemplar file: layer from the path (then globs), technologies from imports. Imports cannot see wrapped clients (a `@/server/core/db/client` wrapper, or Prisma hiding SQL Server): split the TSV by path and pass `--techno` per subset. Next.js repo: repositories got `Next.js,Kysely,Oracle Database`, the rest `Next.js`. NestJS repo, by package: `target/server` → `Nest.js` (+ `SQL Server` for repositories/adapters), `target/webapp` → `Vite`, `target/schemas` → nothing.
...
- The NestJS/React repo's blueprint skill has no `index.sh` (it writes a markdown index instead). The other repo's generic `index.sh` works on it: `bash <repo>/.claude/skills/blueprint/index.sh <out>.md target/server/src target/webapp/src target/schemas/src`.
- In the NestJS repo's project, prod slugs had been derived from names by the seed (`count-endpoint`) while the code uses pattern ids (`controller-count`): a sync would have created duplicates. 28 slugs were renamed by name match (PUT `{slug}`: same id, history kept) before syncing. Result: 84 updated, 15 created; `service-admin-lifecycle-transition` is no longer annotated in code and stays as is.
...
- Stacks match blueprints carrying ANY of their technologies, so a stack must list project-specific technologies only (TypeScript/React/Zod/TanStack Form/Table are shared by both projects). Two prod stacks exist, one per synced project: Next.js + Kysely + Oracle Database (44/44 of its project) and Nest.js + Prisma + SQL Server + Vite + TanStack Router/Query (87/100, 0 leaked). The 12 blueprints of that project's framework-agnostic `schemas` package carry no project-specific technology and stay out of its stack.
...
- Rolled out expand → contract: 0010 added the columns + backfill while the code kept dual-writing the join table, then 0011 added the constraints and dropped it once the new release was live. Prod verified between the two: 144 blueprints across the two synced projects (100 + 44), 0 orphans, no duplicate (project, slug).
...
- A blueprint seeded in March that no code annotates any more (`service-admin-lifecycle-transition`) was deleted from prod; it was part of the old seed corpus, which has since been replaced by synthetic blueprints.
...
- E2E gotcha: `playwright.config.ts` sets `reuseExistingServer: true`, so a stale API left listening on 3002 by another worktree is reused silently — its database is empty, sessions do not resolve, and every UI spec fails on missing elements while the API specs pass. Check `lsof -iTCP:3002 -sTCP:LISTEN` before debugging the app.
- The seed corpus is synthetic and lives in `api/scripts/seed-blueprints.ts`: the 85 markdown files that used to sit in `webapp/src/assets/blueprints/` were excerpts of a client codebase, which has no business being in this repo. Real blueprints live in the registry database only.
